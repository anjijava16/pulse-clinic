 ...
 * @author befast
 */
public class CreateUserListener extends AbstractTableListener {
    ...
    public void actionPerformed(ActionEvent ae) {
        ThreadPoolService.INSTANCE.execute(() -> {
            ...
            final String nfp = String.format("%s %s %s", family, name, father);
            final String base64Password = Base64.encodeBase64String(password.getBytes());            
            
            try {
                final User cachedUser = userService.find(username);
                
                if (cachedUser != null) {
                    ResultToolbarService.INSTANCE.showFailedStatus("Данный логин уже занят");
                    return;
                }
            ...
            try {
                ...
                this.tableService.add(user);
                WindowManager.getInstance().getAccountCreationFrame().clear();
