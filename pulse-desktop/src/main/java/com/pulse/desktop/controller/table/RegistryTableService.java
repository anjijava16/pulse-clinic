 ...
 */
public class RegistryTableService {
    
    private SimpleDateFormat visitFormatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    private SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
    
    public RegistryTableService() {
    }
    
    public String[] proxyFrom(Visit visit, SimpleDateFormat dateFormat) {
        if (visit == null) {
            ResultToolbarService.INSTANCE.showFailedStatus("Информация о визите отсутствует");
        }
        
        final String[] data = new String[TableService.INSTANCE.REGISTRY_TABLE_HEADER.length];
        int ptr = 0;        
        
        Patient patient = PatientService.INSTANCE.getById(visit.getPatientId());
        User account = UserFacade.INSTANCE.findBy(visit.getDoctorId());
        ...
        final String visitDate = visitFormatter.format(visit.getVisitDate());
        
        data[ptr++] = visitDate;
        data[ptr++] = String.valueOf(visit.getId());
        data[ptr++] = String.valueOf(visit.getPatientId());
        data[ptr++] = patient.getNfp();
        data[ptr++] = this.sdf.format(patient.getBirthday()); // String.valueOf(patient.getBirthday());
        data[ptr++] = department;
